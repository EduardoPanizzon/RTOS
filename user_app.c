...
pipe_t pipe;
uint8_t tempo_injecao = 0;
mutex_t buffer_mutex;

TASK tarefa_1()  
{
    while (1) {
        // uint8_t valor = adc_read() >> 2;  
        // write_pipe(&pipe, valor);    
        Nop(); 
        yield();    
    }
}
...
TASK tarefa_2()  
{
    uint8_t valor_pedal = 100;
    while (1) {
        //read_pipe(&pipe, &valor_pedal);  

        
        mutex_lock(&buffer_mutex);
        tempo_injecao = valor_pedal / 20;
